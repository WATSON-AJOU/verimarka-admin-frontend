# Verimarka Admin Frontend Setup

이 문서는 관리자 프론트엔드 로컬 실행 방법을 정리합니다. 기존 상세 인수인계 내용은 [HANDOFF.md](../HANDOFF.md)를 참고하세요.
...
관리자 프론트 개발 서버는 `/api` 요청을 `http://127.0.0.1:8000` 백엔드로 프록시합니다. 먼저 백엔드를 실행합니다.
...
로컬 API 호출은 Vite proxy를 사용하므로 `VITE_API_BASE_URL`을 생략할 수 있습니다. OAuth 로그인을 테스트할 때만 `.env.local`에 클라이언트 ID를 설정합니다.
