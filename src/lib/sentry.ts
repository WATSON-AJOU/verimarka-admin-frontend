...
export async function initSentry() {
  ...
  try {
    ...
    window.Sentry.setTag?.("app", "admin-frontend");
    window.Sentry.setTag?.("surface", "admin");

    appLogger.info("sentry.admin_initialized", {
      environment: import.meta.env.VITE_SENTRY_ENVIRONMENT || import.meta.env.MODE,
    });
  } catch (error) {
    appLogger.error("sentry.admin_init_failed", {
      error,
    });
  }
}
